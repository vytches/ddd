...
1. [x] `resolve()` checks the adapter's own registry first and falls back to
...
2. [x] Cache `design:paramtypes` lazily, once per constructor, on first
...
4. [x] `createScope()` no longer duplicates the full singleton descriptor set
       per scope (share by reference / lazy view).
5. [x] Consumer-side regression measurement (in `juz-ide-api` or a
       representative NestJS fixture): first-time resolve and cold start tracked
       and documented.
6. [x] Backward-compat: adapter public behavior preserved (resolution results,
       error types, scope semantics). Assess `@vytches/nestjs` surface
       separately from the `@vytches/ddd-di` snapshot.

## Closure note (task-tidy reconciliation, 2026-07-18)

Merged to `develop` in `perf/VP-006b-nestjs-adapter` (`9b56a71a`, merge
`cf4029dd`, 2026-07-12): registry-first resolve, lazy paramtypes cache, COW
scopes. `library-quality-verifier` recorded a GO verdict same day (`3b81c5ba`):
232/232 tests green, matches approved decisions, no `packages/di` edits. Task
file was left at `status: backlog` with ACs unchecked after merge — flagged by
`/pulse` across two consecutive runs; reconciled here against git history rather
than re-verifying the implementation.
