...
Create a **DDD-native seeding framework** that deeply integrates with VytchesDDD
architecture, respecting domain boundaries, aggregates, events, and all DDD
patterns while providing excellent developer experience for all test types.

### Decision: Package Strategy

**Start in `@vytches/ddd-testing`** for rapid iteration and dogfooding, then
**migrate to `@vytches/ddd-seeder`** after v1.0 stability.
...
export class DDDFactory<T> implements IDDDFactory<T> {
  ...
  constructor(definition: IFactoryDefinition<T>, repository?: IRepository<T>) {
  ...
  async createMany(count: number, overrides?: DeepPartial<T>): Promise<T[]> {
    ...
    for (let i = 0; i < count; i += BATCH_SIZE) {
      const batch = this.buildMany(Math.min(BATCH_SIZE, count - i), overrides);
...
export class AggregateFactory<T extends IAggregateRoot> extends DDDFactory<T> {
   ...
   */
  buildWithHistory(builder: (agg: T) => T, overrides?: DeepPartial<T>): T {
...
export class RepositorySeeder<T extends IAggregateRoot> {
  ...
  constructor(repository: IBaseRepository<T>, factory: AggregateFactory<T>) {
    this.repository = repository;
   ...
   */
  async seedForQuery(count: number, queryPattern: Partial<T>): Promise<T[]> {
  ...
  async buildScenario(config: {
    ...
    for (let i = 0; i < config.steps.length; i++) {
      ...
      scenario.steps.push({
        ...
        expectedCompensation: this.getCompensationEvent(
          stepConfig.expectedEvent
        ),
      });
    }
...
export class VolumeGenerator<T> {
   ...
   */
  generateWithDistribution(count: number, distribution: Distribution): T[] {
    const results: T[] = [];
   ...
   */
  async generateConcurrent(count: number, concurrency: number): Promise<T[]> {
    const results: T[] = [];
  ...
  private applyDistribution(
    ...
    switch (distribution.type) {
      case 'normal':
        return this.applyNormalDistribution(entity, distribution, percentile);

      case 'pareto':
        return this.applyParetoDistribution(entity, distribution, percentile);
  ...
  async insertMany<T>(table: string, data: T[]): Promise<T[]> {
    ...
    data.forEach((item, index) => {
      const rowPlaceholders = keys
        .map((_, keyIndex) => `$${index * keys.length + keyIndex + 1}`)
        .join(', ');
...
export class PenetrationTestAdapter implements ISeederProvider {
  ...
  async execute<T>(operation: ProviderOperation<T>): Promise<T> {
    ...
    const exploits = await this.exploitDB.getExploits(
      operation.options?.target
    );
...
export class FactoryRegistry {
  ...
  autoRegister(aggregates: Array<typeof AggregateRoot>): void {
    for (const AggregateClass of aggregates) {
      ...
      if (metadata) {
        const factory = this.createFactoryFromMetadata(
          AggregateClass,
          metadata
        );
      ...
      build: (overrides?: DeepPartial<T>) => {
        ...
        for (const relation of relations) {
          ...
          if (relation.type === 'hasMany') {
            entity[relation.property] = relatedFactory.buildMany(
              relation.count
            );
...
test('user registration', async () => {
  const user = await userFactory.use('premium').state('active').create();
    ...
    steps: [
      {
        aggregate: 'Payment',
        operation: 'process',
        expectedEvent: 'PaymentProcessed',
      },
      {
        aggregate: 'Inventory',
        operation: 'reserve',
        expectedEvent: 'InventoryReserved',
      },
      {
        aggregate: 'Shipping',
        operation: 'schedule',
        expectedEvent: 'ShippingScheduled',
      },
    ],
...
This DDD Seeder will be a **game-changer** for testing VytchesDDD applications.
By deeply integrating with the framework's patterns and respecting domain
boundaries, it provides a testing experience that's both powerful and intuitive.

The phased approach ensures we can start using it immediately in the testing
package while planning for a clean separation once stable. This gives us the
best of both worlds: rapid iteration and clean architecture.
