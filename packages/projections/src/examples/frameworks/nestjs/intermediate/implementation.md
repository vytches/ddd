...
The bridge pattern allows seamless integration between NestJS and VytchesDDD DI
systems without creating conflicting instances:
...
export class AdvancedProjectionManagerDomainService extends ProjectionEngine {
  ...
  private initializeRegistry(): void {
    ...
    this.registry.on(
      ...
      (projection: ProjectionBase<any>) => {
        ...
        console.log(
          `Enterprise projection registered: ${projection.projectionName}`
        );
      }
    );
    ...
    this.registry.on(
      'projectionHealthChanged',
      (projectionName: string, health: any) => {
  ...
  private enhanceProjectionWithEnterpriseFeatures(
    projection: ProjectionBase<any>
  ): void {
    // Add monitoring
  ...
  async processEvent(event: IDomainEvent): Promise<ServiceResponse<any>> {
    try {
      ...
      const prioritizedProjections = this.prioritizeProjections(
        relevantProjections,
        event
      );
  ...
  getEnterpriseHealthStatus(): any {
    const projectionHealths = Array.from(this.activeProjections.values()).map(
      projection => ({
  ...
  private async getRelevantProjections(
    event: IDomainEvent
  ...
  private async isProjectionRelevant(
    projection: ProjectionBase<any>,
    event: IDomainEvent
  ): Promise<boolean> {
  ...
  private handleProjectionHealthChange(
...
@Module({
  ...
  controllers: [ProjectionAnalyticsController, ProjectionHealthController],
...
export class AdvancedProjectionModule implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      ...
      const manager =
        VytchesDDD.resolve<AdvancedProjectionManagerDomainService>(
          'advancedProjectionManager'
        );
      ...
      if (!result.success) {
        throw new Error(
          `Failed to start enterprise projections: ${result.error?.message}`
        );
      }

      console.log(
        'Advanced Projection Module initialized with VytchesDDD integration'
      );
    } catch (error) {
  ...
  async onModuleDestroy() {
    try {
      ...
      if (!result.success) {
        console.error(
          'Failed to stop enterprise projections:',
...
export class MyProjectionService {
  ...
  constructor() {
    this.domainService =
      VytchesDDD.resolve<MyProjectionDomainService>('myProjection');
  }
...
export class EnterpriseProjectionService {
  ...
  constructor() {
    this.domainService = VytchesDDD.resolve<EnterpriseProjectionDomainService>(
