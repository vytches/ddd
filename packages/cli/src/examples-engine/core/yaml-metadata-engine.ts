...
export class YamlMetadataEngine {
  ...
  async loadHierarchicalMetadata(config: MetadataConfig): Promise<ResolvedMetadata | null> {
    console.log(
      `[yaml-metadata-engine] Loading hierarchical metadata for ${config.packageName}/${config.className}${config.methodName ? `/${config.methodName}` : ''}`
    );
    ...
    console.log(
      `[yaml-metadata-engine] Resolved metadata with ${Object.keys(resolvedMetadata).length} keys: ${Object.keys(resolvedMetadata).join(', ')}`
    );
  ...
  private async collectMetadataSources(config: MetadataConfig): Promise<MetadataSource[]> {
    ...
    if (config.methodName) {
      const methodMetadata = await this.loadMethodMetadata(
        config.packageName,
        config.className,
        config.methodName
      );
  ...
  private async loadMethodMetadata(
    ...
    try {
      ...
      console.log(
        `[yaml-metadata-engine] Could not load method metadata for ${methodName}:`,
        error
      );
      return null;
    }
  ...
  private async loadAllMethodsFromClass(
    packageName: string,
    className: string
  ...
  private parseMetadata(content: string): MetadataParseResult {
    ...
    try {
      ...
      console.log(
        `[yaml-metadata-engine] YAML parse failed, trying @ tags:`,
