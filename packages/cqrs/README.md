...
> **Enterprise-grade CQRS implementation with decorators, middleware, and automatic handler discovery**

Complete Command Query Responsibility Segregation (CQRS) implementation with decorator-based handler registration, execution context, middleware pipeline, and seamless integration with dependency injection.
...
CQRS is a pattern that separates read and write operations for a data store. Commands perform updates and queries return data.
...
export class GetUserByIdQueryHandler implements IQueryHandler<GetUserByIdQuery, UserDto> {
  ...
  async handle(query: GetUserByIdQuery, context: ExecutionContext): Promise<UserDto> {
...
export class UpdateUserProfileCommandHandler 
  ...
  async handle(command: UpdateUserProfileCommand, context: ExecutionContext): Promise<void> {
    // Validate command
...
export class GetUsersByRoleQueryHandler 
  ...
  async handle(query: GetUsersByRoleQuery, context: ExecutionContext): Promise<UserDto[]> {
...
export class GetUserOrderHistoryQueryHandler 
  ...
  async handle(query: GetUserOrderHistoryQuery, context: ExecutionContext): Promise<UserOrderHistoryDto> {
    // Load user
    const user = await this.userRepository.findById(query.userId);
    if (!user) {
...
export class CreateOrderCommandHandler {
  async handle(command: CreateOrderCommand, context: ExecutionContext): Promise<void> {
...
export class GetOrderDetailsQueryHandler {
  async handle(query: GetOrderDetailsQuery, context: ExecutionContext): Promise<OrderDetailsDto> {
...
export class ProcessOrderCommandHandler {
  async handle(command: ProcessOrderCommand, context: ExecutionContext): Promise<void> {
...
export class ComplexBusinessCommandHandler {
  async handle(command: ComplexBusinessCommand, context: ExecutionContext): Promise<void> {
...
export class ProcessPaymentCommandHandler {
  async handle(command: ProcessPaymentCommand, context: ExecutionContext): Promise<void> {
...
export class StartOrderProcessingSagaCommandHandler {
  async handle(command: StartOrderProcessingSagaCommand, context: ExecutionContext): Promise<void> {
...
export class ProcessOrderCommandHandler {
  async handle(command: ProcessOrderCommand, context: ExecutionContext): Promise<void> {
    ...
    for (const event of events) {
      const followUpCommands = await this.commandGeneratorService.generateCommands(event);
...
export class GetOrderSummaryQueryHandler {
  async handle(query: GetOrderSummaryQuery, context: ExecutionContext): Promise<OrderSummaryDto> {
    ...
    const [order, customer, payments] = await Promise.all([
      ...
      this.queryBus.execute(new GetCustomerDetailsQuery(query.customerId), context),
...
export class ProcessLargeDataCommandHandler {
  async handle(command: ProcessLargeDataCommand, context: ExecutionContext): Promise<void> {
...
export class GetProductCatalogQueryHandler {
  ...
  async handle(query: GetProductCatalogQuery, context: ExecutionContext): Promise<ProductCatalogDto> {
    ...
    // Load from database
    const catalog = await this.catalogService.getProductCatalog(query.categoryId);
...
describe('CreateUserCommandHandler', () => {
  ...
  it('should handle validation errors', async () => {
    ...
    // Act & Assert
    await expect(handler.handle(command, mockContext))
...
describe('Order Management Integration', () => {
  ...
  beforeEach(() => {
    ...
    // Register handlers
    commandBus.registerHandler(CreateOrderCommand, new CreateOrderCommandHandler());
...
```typescript
import { createMockCommandBus, createMockQueryBus } from '@vytches-ddd/cqrs/testing';
  ...
  async handle(command: ProcessPaymentCommand, context: ExecutionContext): Promise<void> {
    try {
      ...
      if (error instanceof InsufficientFundsError) {
        throw new PaymentFailedError('Insufficient funds', command.paymentDetails.amount);
      }
      
      if (error instanceof InvalidCardError) {
        throw new PaymentFailedError('Invalid card details', command.paymentDetails.cardNumber);
      }
...
export class OrderCreatedEventHandler {
  async handle(event: OrderCreatedEvent, context: ExecutionContext): Promise<void> {
...
export class StartOrderProcessingSagaCommandHandler {
  async handle(command: StartOrderProcessingSagaCommand, context: ExecutionContext): Promise<void> {
    ...
    // Define saga steps
    const steps = [
      () => this.commandBus.execute(new ValidateOrderCommand(command.orderId), context),
      () => this.commandBus.execute(new ReserveInventoryCommand(command.orderId), context),
      () => this.commandBus.execute(new ProcessPaymentCommand(command.orderId), context),
      () => this.commandBus.execute(new FulfillOrderCommand(command.orderId), context)
    ];
