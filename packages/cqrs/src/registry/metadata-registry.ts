...
type CommandHandlerConstructor<T extends ICommand = ICommand> = new (
  ...args: any[]
) => ICommandHandler<T>;
type QueryConstructor<T extends IQuery<unknown> = IQuery<unknown>> = new (...args: any[]) => T;
type QueryHandlerConstructor<T extends IQuery<R>, R = unknown> = new (
  ...args: any[]
) => IQueryHandler<T, R>;

export class CQRSMetadataRegistry {
  private static commandHandlers = new Map<CommandConstructor, CommandHandlerConstructor>();
  private static queryHandlers = new Map<
    QueryConstructor,
    QueryHandlerConstructor<IQuery<unknown>, unknown>
  >();
  ...
  static registerQueryHandler<T extends IQuery<R>, R>(
    ...
    this.queryHandlers.set(
      queryType,
      handlerType as QueryHandlerConstructor<IQuery<unknown>, unknown>
    );
  }
