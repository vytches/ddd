...
High-frequency trading platform requiring atomic multi-account transactions,
real-time position tracking, sophisticated risk queries, and multi-tenant broker
support.
...
class RiskSpecificationFactory {
  ...
  static createForPortfolioRebalance(
    portfolio: string
  ): CompositeSpecification<Position> {
    return new ActivePositionsSpec()
...
Multi-provider healthcare platform managing patient records, treatment
histories, insurance claims, and regulatory compliance across different
healthcare networks.
...
class HealthcareUnitOfWork extends UnitOfWork {
  async processPatientTreatment(
    ...
    try {
      ...
      await this.updateProviderAvailability(
        episode.providerId,
        episode.duration
      );
...
Global supply chain platform managing inventory across multiple warehouses,
supplier relationships, demand forecasting, and logistics optimization.
...
class InventorySpecificationFactory {
  static createForOptimalAllocation(
    ...
    let spec = new AvailableInventorySpec(productId, quantity).and(
      new QualityControlPassedSpec()
    );

    switch (priority) {
      case 'urgent':
        spec = spec
          .and(new NearestLocationSpec())
          .and(new ExpressFulfillmentCapableSpec());
        break;
      case 'bulk':
        spec = spec
          .and(new BulkHandlingCapableSpec())
...
University system managing student enrollments, course scheduling, faculty
assignments, and academic records across multiple campuses and departments.
...
class AcademicUnitOfWork extends UnitOfWork {
  async processStudentEnrollment(
    ...
    try {
      ...
      const eligibilitySpec =
        AcademicSpecificationFactory.createForEnrollment(enrollment);
...
- **Specification Pattern**: Enables sophisticated querying and business rule
  evaluation
- **Multi-Tenant Repository**: Provides secure data isolation and
  tenant-specific features

The key to successful implementation lies in understanding when and how to
combine these patterns based on specific business requirements, performance
