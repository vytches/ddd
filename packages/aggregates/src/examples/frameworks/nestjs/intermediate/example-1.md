...
**Focus**: Event sourced shopping cart with NestJS and @vytches-ddd/di integration  
**Base Example**: [Event Sourced Shopping Cart](../../intermediate/example-1.md)
**Dependencies**: @nestjs/common, @vytches-ddd/aggregates, @vytches-ddd/di, @vytches-ddd/events
...
export class EventSourcedCartService {
  ...
  async createCart(customerId: string, currency: string = 'USD'): Promise<ShoppingCart> {
    try {
      // Get EventSourcedShoppingCartAggregate from VytchesDDD
      const CartAggregateClass = VytchesDDD.resolve<any>('EventSourcedShoppingCartAggregate');
      ...
      this.logger.log(`Event sourced cart created: ${cart.id} for customer ${customerId}`);
      return cart;
    } catch (error) {
      this.logger.error(`Failed to create event sourced cart: ${error.message}`);
      throw error;
    }
  }
  ...
  async getCartFromEventHistory(cartId: string): Promise<ShoppingCart> {
    try {
      ...
      this.logger.log(`Cart reconstructed from ${events.length} events: ${cartId}`);
      return cart;
    } catch (error) {
      this.logger.error(`Failed to reconstruct cart from events: ${error.message}`);
      throw error;
    }
  }
  ...
  async getCartWithSnapshotOptimization(cartId: string): Promise<ShoppingCart> {
    try {
      ...
      const eventsAfterSnapshot = await this.getEventsAfterSnapshot(cartId, snapshotData?.version || 0);
      ...
      if (snapshotData) {
        ...
        this.logger.log(`Cart loaded from snapshot (v${snapshotData.version}) + ${eventsAfterSnapshot.length} events`);
      } else {
        ...
        this.logger.log(`Cart loaded from full event history: ${allEvents.length} events`);
      }
      
      return cartAggregate.toSnapshot();
    } catch (error) {
      this.logger.error(`Failed to load cart with snapshot optimization: ${error.message}`);
      throw error;
    }
  }
  ...
  async addItemToCart(cartId: string, item: CartItem): Promise<ShoppingCart> {
    try {
      ...
      await this.persistCartEvents(cartId, cartAggregate.getUncommittedEvents());
      
      const updatedCart = cartAggregate.toSnapshot();
      
      this.logger.log(`Item added to cart ${cartId}: ${item.productId} x${item.quantity}`);
      return updatedCart;
    } catch (error) {
  ...
  async updateItemQuantity(cartId: string, productId: string, quantity: number): Promise<ShoppingCart> {
    try {
      ...
      this.logger.log(`Item quantity updated in cart ${cartId}: ${productId} -> ${quantity}`);
      return updatedCart;
    } catch (error) {
  ...
  async applyDiscountCode(cartId: string, discountCode: string): Promise<ShoppingCart> {
    try {
      const cartAggregate = await this.loadEventSourcedCart(cartId);
      ...
      this.logger.log(`Discount code applied to cart ${cartId}: ${discountCode}`);
      return updatedCart;
    } catch (error) {
  ...
  async updateShippingMethod(cartId: string, shippingMethod: ShippingMethod): Promise<ShoppingCart> {
    try {
      const cartAggregate = await this.loadEventSourcedCart(cartId);
      ...
      this.logger.log(`Shipping method updated for cart ${cartId}: ${shippingMethod.type}`);
      return updatedCart;
    } catch (error) {
  ...
  async calculateCartTotals(cartId: string): Promise<PriceCalculation> {
    try {
      ...
      this.logger.log(`Cart totals calculated for ${cartId}: ${calculation.grandTotal}`);
      return calculation;
    } catch (error) {
  ...
  async getCartAtPoint(cartId: string, timestamp: Date): Promise<ShoppingCart> {
    try {
      ...
      const eventsUpToTimestamp = await this.getCartEventsUpTo(cartId, timestamp);
      ...
      this.logger.log(`Historical cart state retrieved for ${cartId} at ${timestamp.toISOString()}`);
      return historicalCart;
    } catch (error) {
      this.logger.error(`Failed to get cart at point in time: ${error.message}`);
      throw error;
    }
  }
  ...
  async getCartVersionHistory(cartId: string): Promise<CartSnapshot[]> {
    try {
      ...
      this.logger.log(`Cart version history retrieved: ${snapshots.length} snapshots`);
      return snapshots;
    } catch (error) {
  ...
  async createSnapshot(cartId: string): Promise<boolean> {
    try {
      ...
      this.logger.log(`Snapshot created for cart ${cartId} at version ${snapshot.version}`);
      return true;
    } catch (error) {
  ...
  async getCartEventStream(cartId: string): Promise<any[]> {
    try {
      ...
      this.logger.log(`Cart event stream retrieved: ${allEvents.length} events`);
      return allEvents;
    } catch (error) {
  ...
  async getCartAnalytics(cartId: string): Promise<any> {
    try {
      ...
      const analytics = {
        ...
        priceRecalculations: events.filter(e => e.eventType === 'PriceRecalculated').length,
        discountsApplied: events.filter(e => e.eventType === 'DiscountApplied').length,
        firstActivity: events.length > 0 ? events[0].occurredAt : null,
        lastActivity: events.length > 0 ? events[events.length - 1].occurredAt : null,
  ...
  private async getCartEventHistory(cartId: string): Promise<any[]> {
    ...
    return [
      ...
      { eventType: 'ItemAdded', cartId, productId: 'product-1', quantity: 2, occurredAt: new Date() }
    ];
  }

  private async getEventsAfterSnapshot(cartId: string, version: number): Promise<any[]> {
    // Mock implementation
    return [];
  }
  ...
  private async getCartEventsUpTo(cartId: string, timestamp: Date): Promise<any[]> {
    const allEvents = await this.getCartEventHistory(cartId);
    return allEvents.filter(event => event.occurredAt <= timestamp);
  }

  private async persistCartEvents(cartId: string, events: any[]): Promise<void> {
  ...
  private async persistSnapshot(cartId: string, snapshot: any): Promise<void> {
    // Mock implementation
    this.logger.debug(`Persisting snapshot for cart ${cartId} at version ${snapshot.version}`);
  }

  private isSignificantEvent(event: any): boolean {
    return ['CartCreated', 'DiscountApplied', 'CartCleared'].includes(event.eventType);
  }
  ...
  private calculateAverageSessionTime(events: any[]): number {
    ...
    return (lastEvent.occurredAt.getTime() - firstEvent.occurredAt.getTime()) / (1000 * 60); // minutes
  }

  private calculateAbandonmentRisk(events: any[]): number {
    // Simple risk calculation based on event patterns
    const lastEventTime = events.length > 0 ? events[events.length - 1].occurredAt : new Date();
    const timeSinceLastActivity = (Date.now() - lastEventTime.getTime()) / (1000 * 60); // minutes
