...
**Patterns**: Aggregate Root, Domain Events, Invariant Protection
**Dependencies**: @vytches-ddd/aggregates, @vytches-ddd/domain-primitives, @vytches-ddd/contracts

## Description

This example demonstrates a basic aggregate root implementation for user management. The aggregate enforces business rules, maintains consistency, and emits domain events for state changes.

## Business Context

A user management system needs to maintain user profiles with strict business rules: unique usernames, valid email formats, and activity tracking. The aggregate ensures all invariants are protected and changes are properly recorded through domain events.
...
export class UserAggregate extends AggregateRoot {
  ...
  reactivate(): void {
    ...
    this.addDomainEvent(new UserUpdatedEvent(this.id.value, { isActive: true }));
  }
  ...
  private validateUsername(username: string): void {
    ...
    if (!usernameRegex.test(username)) {
      throw new InvalidUserDataError('Username can only contain letters, numbers, underscores and hyphens');
    }
  }
  ...
  private ensureActive(): void {
    if (!this.isActive) {
      throw new BaseError('USER_INACTIVE', 'Cannot perform operations on inactive user');
    }
  }
...
2. **Username Constraints**: 3-30 characters, alphanumeric with underscores/hyphens
