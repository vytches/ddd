...
Intermediate **Domain**: Banking & Financial Services **Patterns**: Capability
Pattern, Command-Query Separation, Optimistic Locking, Transaction History
**Dependencies**: @vytches/ddd-aggregates, @vytches/ddd-domain-primitives,
@vytches/ddd-contracts

## Description

This example demonstrates an advanced banking account aggregate that uses the
capability pattern to separate concerns, implements sophisticated business rules
for financial transactions, and maintains detailed audit trails with optimistic
locking for concurrent operations.

## Business Context

A digital banking platform requires robust account management with features like
transaction limits, automatic categorization, fraud detection triggers, and
regulatory compliance. The aggregate must handle high-volume concurrent
transactions while maintaining data integrity and providing comprehensive audit
capabilities.
...
export class TransactionCapability implements ITransactionCapability {
  ...
  async processTransaction(
    ...
    const fullTransaction: Transaction = {
      ...
      balance:
        this.aggregate.getCurrentBalance() +
...
export class RiskCapability implements IRiskCapability {
  ...
  assessRisk(transaction: Omit<Transaction, 'id' | 'balance'>): RiskAssessment {
    ...
    if (
      ...
      this.isUnusualLocation(transaction.metadata.location)
    ) {
      riskScore += 40;
  ...
  checkFraudPatterns(transactions: Transaction[]): boolean {
    ...
    const identicalAmounts = amounts.filter(
      (amount, index) => amounts.indexOf(amount) !== index
    );
  ...
  updateRiskProfile(transaction: Transaction): void {
    ...
    this.riskProfile.factors = [
      ...new Set([...this.riskProfile.factors, ...assessment.factors]),
    ];
  }
...
export class ComplianceCapability implements IComplianceCapability {
  ...
  validateCompliance(transaction: Omit<Transaction, 'id' | 'balance'>): void {
    ...
    if (dailyTotal > 50000) {
      ...
      throw new BaseError(
        ...
        'Daily transaction limit exceeded - potential structuring'
      );
    }
  }
  ...
  reportSuspiciousActivity(transaction: Transaction): void {
    ...
    console.log(
      `SAR filed for transaction ${transaction.id}: ${transaction.description}`
    );
  }
  ...
  private checkSanctionsList(recipient: string): void {
    ...
    if (sanctionedEntities.some(entity => recipient.includes(entity))) {
      throw new BaseError(
        'SANCTIONS_VIOLATION',
        'Transaction with sanctioned entity blocked'
      );
    }
  }
}
...
export class AdvancedBankingAccountAggregate extends AggregateRoot {
  ...
  async deposit(
    ...
    const transactionId =
      await this.transactionCapability.processTransaction(transaction);
  ...
  freezeAccount(reason: string, frozenBy: string): void {
    ...
    this.addDomainEvent(
      new AccountFrozenEvent(this.id.value, reason, frozenBy, new Date())
    );
  }
  ...
  getAccountSummary(): any {
    ...
    const fraudPatterns =
      this.riskCapability.checkFraudPatterns(recentTransactions);
...
export function advancedBankingExample(): void {
  ...
  account
    .deposit(1000, 'Salary deposit', 'PAY-001')
    .then(txnId => console.log('Deposit completed:', txnId));

  account
    .withdraw(200, 'ATM withdrawal', 'ATM-123')
    .then(txnId => console.log('Withdrawal completed:', txnId));

  account
    .transfer(500, 'ACC-789', 'Transfer to savings')
...
- **Capability Pattern**: Separation of concerns with dedicated capability
  classes
...
- **Business Rule Enforcement**: Transaction limits, status validation, and
  constraints
