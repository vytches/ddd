...
Complete aggregate root implementation with built-in event sourcing, capability system, and automatic event management. Designed for complex domain models with strong consistency boundaries.
...
An aggregate root is the only member of an aggregate that outside objects are allowed to hold references to. It's the gatekeeper for the aggregate, ensuring consistency and managing the lifecycle of the aggregate.
...
Event sourcing is a pattern where state changes are stored as a sequence of events. The current state is derived by replaying these events.
...
Capabilities are modular extensions that can be added to aggregates to provide cross-cutting concerns like auditing, caching, or validation.
...
class OrderAggregate extends AggregateRoot {
  ...
  // Business methods
  addItem(productId: EntityId, quantity: number, price: number): Result<void, Error> {
...
```typescript
import { EventSourcedAggregate, EventHandlerMap } from '@vytches-ddd/aggregates';
...
class ProductAggregate extends AggregateRoot {
  ...
  adjustStock(quantity: number): Result<void, Error> {
    ...
    this.addDomainEvent(new ProductStockAdjustedEvent({
      productId: this.id.value,
      adjustment: quantity,
      newStock,
      adjustedAt: new Date()
...
class OrderAggregate extends AggregateRoot {
  ...
  updateShippingAddress(newAddress: Address): Result<void, Error> {
    ...
    this.addDomainEvent(new OrderShippingAddressUpdatedEvent({
      orderId: this.id.value,
...
class OrderRepository {
  async save(order: OrderAggregate): Promise<void> {
    ...
    if (events.length >= this.snapshotThreshold) {
      await this.snapshotStore.createSnapshot(order.id.value, order.toSnapshot());
    }
  }
}
```
...
class OrderAggregate extends AggregateRoot {
  ...
  private validateInvariants(): void {
    ...
    if (this.status === OrderStatus.CONFIRMED && this.totalAmount === 0) {
      throw new InvariantViolationError('Confirmed orders must have positive total');
    }
...
3. **Enforce Invariants at Aggregate Boundaries**
   ...
   class OrderAggregate extends AggregateRoot {
     ...
     private static readonly MIN_TOTAL = 10.00;
     
     addItem(item: OrderItem): Result<void, Error> {
       // Validate aggregate invariants
       if (this.items.length >= OrderAggregate.MAX_ITEMS) {
         return Result.fail(new Error(`Order cannot have more than ${OrderAggregate.MAX_ITEMS} items`));
       }
       
       const newTotal = this.calculateTotalWithItem(item);
       if (this.status === OrderStatus.PENDING && newTotal < OrderAggregate.MIN_TOTAL) {
         return Result.fail(new Error(`Order total must be at least ${OrderAggregate.MIN_TOTAL}`));
       }
...
3. **Batching Operations**
   ...
   class OrderAggregate extends AggregateRoot {
     addItems(items: OrderItem[]): Result<void, Error> {
       ...
       this.addDomainEvent(new OrderItemsBatchAddedEvent({
         orderId: this.id.value,
...
const account = new AggregateTestBuilder(BankAccountAggregate)
  .fromEventStream([
    ...
    new MoneyDepositedEvent({ accountId: 'account-123', amount: 500, balance: 1500 })
  ])
  .build();
