...
import { VersionedACLRegistry } from '../src/versioned-acl-registry';
  ...
  describe('registerVersioned method', () => {
    ...
    it('should reject invalid version format', async () => {
      ...
      const [error] = await safeRun(() =>
        registry.registerVersioned('TestContext', 'invalid', adapter)
      );
    ...
    it('should reject invalid semantic version formats', async () => {
      ...
      // Act
      const [error] = await safeRun(() =>
        registry.registerVersioned('TestContext', '1.0', adapter)
      );
      const [error2] = await safeRun(() =>
        registry.registerVersioned('TestContext', 'v1.0.0', adapter)
      );
      const [error3] = await safeRun(() =>
        registry.registerVersioned('TestContext', '1.0.0.0', adapter)
      );
    ...
    describe('getVersions method', () => {
      ...
      it('should handle pre-release versions in sorting', () => {
        const versions = ['1.0.0-alpha', '1.0.0', '1.0.0-beta', '1.0.1-alpha', '1.0.1'];
    ...
    describe('version comparison and sorting', () => {
      it('should handle complex version comparisons correctly', () => {
        const versions = ['1.0.0', '1.0.1', '1.0.2', '1.0.10', '1.1.0', '1.2.0', '1.10.0', '2.0.0'];
    ...
    describe('edge cases and error handling', async () => {
      ...
      it('should handle version with leading zeros', async () => {
        ...
        // Act
        const [error] = await safeRun(() =>
          registry.registerVersioned('TestContext', '1.01.0', adapter)
        );
      ...
      it('should handle empty pre-release tag', async () => {
        ...
        // Act
        const [error] = await safeRun(() =>
          registry.registerVersioned('TestContext', '1.0.0-', adapter)
        );
