  ...
  implements IACLAdapter<TDomainModel, TExternalModel, TResult>
{
  ...
  async fetch(identifier: string): Promise<Result<TDomainModel, ACLError>> {
    try {
      ...
      internalLogger.error('ACL fetch failed', error instanceof Error ? error : undefined, {
        identifier,
  ...
  protected async executeCore(
    ...
    try {
      if (!this.supportsOperation(operation)) {
        internalLogger.warn('ACL: unsupported operation attempted', {
          operation,
          context: this.contextInfo.contextName,
      ...
      internalLogger.error('ACL operation failed', error instanceof Error ? error : undefined, {
        operation,
        context: this.contextInfo.contextName,
        error: error instanceof Error ? error.message : String(error),
