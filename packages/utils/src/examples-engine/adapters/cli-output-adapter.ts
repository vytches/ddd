...
export class CliOutputAdapter {
  ...
  format(metadata: ResolvedMetadata): string {
    console.log(`[cli-output-adapter] Formatting metadata with ${Object.keys(metadata).length} keys`);
  ...
  private addCustomSections(sections: string[], metadata: ResolvedMetadata): void {
    ...
    const processedFields = new Set([
      'description', 'businessContext', 'author', 'since', 'license',
      'tags', 'warnings', 'examples'
    ]);
