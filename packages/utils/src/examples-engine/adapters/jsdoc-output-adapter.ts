...
export class JsDocOutputAdapter {
  ...
  format(metadata: ResolvedMetadata): string {
    console.log(`[jsdoc-output-adapter] Formatting metadata with ${Object.keys(metadata).length} keys`);
  ...
  private addCustomTags(lines: string[], metadata: ResolvedMetadata): void {
    ...
    const processedFields = new Set([
      'description', 'businessContext', 'author', 'since', 'deprecated',
      'see', 'link', 'todo', 'note', 'license', 'warnings', 'tags', 'examples'
    ]);
