...
This example demonstrates automatic registration of CQRS command and query handlers using enhanced decorators. The DI system automatically discovers and registers handlers, making them available through the command and query buses without manual registration.

## Business Context

In CQRS architectures, commands and queries are handled by separate handlers. Rather than manually registering each handler, the auto-discovery system finds decorated handlers and makes them available through the appropriate buses, ensuring consistent registration and reducing configuration errors.
...
export class CreateUserHandler implements ICommandHandler<CreateUserCommand> {
  ...
  async handle(command: CreateUserCommand): Promise<void> {
    ...
    const auditService = VytchesDDD.resolve<AuditService>('auditService', 'UserManagement');
...
export class UpdateUserHandler implements ICommandHandler<UpdateUserCommand> {
  ...
  async handle(command: UpdateUserCommand): Promise<void> {
    ...
    console.log(`UpdateUserHandler: Processing update for user ${command.userId}`);
...
export class GetUserHandler implements IQueryHandler<GetUserQuery, User> {
  ...
  async handle(query: GetUserQuery): Promise<User | null> {
    ...
    const cacheService = VytchesDDD.resolve<CacheService>('cacheService', 'UserManagement');
...
export class GetUsersHandler implements IQueryHandler<GetUsersQuery, PaginatedResponse<User>> {
  /**
  ...
  async handle(query: GetUsersQuery): Promise<PaginatedResponse<User>> {
    ...
    console.log(`GetUsersHandler: Processing paginated query - Page ${query.pagination.page}`);
  ...
  private async fetchUsersFromDatabase(pagination: PaginationParams): Promise<User[]> {
    // Simulate database fetch with pagination
    console.log(`Fetching users - Page ${pagination.page}, Limit ${pagination.limit}`);
...
async function demonstrateCQRSHandlerRegistration(): Promise<void> {
  ...
  const updateCommand = new UpdateUserCommand('user_123', updateUserData, 'req-002');
  ...
  console.log(`Retrieved ${users.data.length} users, Total: ${users.pagination.total}`);
  
  console.log('\n4. Handler metadata:');
  const createHandlerMetadata = VytchesDDD.getHandlerMetadata('CreateUserHandler');
...
- **Missing Handler Registration**: Ensure handlers are decorated with `@CommandHandler` or `@QueryHandler`
...
- [Framework Integration Patterns](../advanced/example-1.md) - Advanced framework integration