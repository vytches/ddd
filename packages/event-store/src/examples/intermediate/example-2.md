...
**Domain**: Architecture
**Patterns**: stream-processing, read-models, projection-building, cqrs-read-side
**Dependencies**: @vytches-ddd/event-store, @vytches-ddd/projections, @vytches-ddd/events

## Description

Building read models and projections directly from event streams with real-time updates, sophisticated aggregation patterns, and efficient query optimization. This example demonstrates advanced projection strategies for complex business intelligence and reporting requirements.

## Business Context

Modern applications need multiple views of the same data for different purposes - operational dashboards, analytics reports, search indices, and customer-facing displays. Stream-based projections allow real-time maintenance of these views while keeping the write-side optimized for business operations.
...
export class StreamProjectionEngine {
  ...
  async buildOrderSummaryProjection(): Promise<Result<OrderSummaryProjection, Error>> {
    try {
      ...
      for (const streamId of orderStreams) {
        ...
        if (orderProjection.isSuccess()) {
          ...
          this.projectionStore.set(`order-summary-${orderId}`, orderProjection.value);
        }
      }
      ...
      return Result.fail(new Error(`Order summary projection failed: ${error.message}`));
    }
  }

  async buildSingleOrderProjection(orderId: string): Promise<Result<OrderSummaryProjection, Error>> {
    try {
      ...
      return Result.fail(new Error(`Single order projection failed: ${error.message}`));
    }
  }
  ...
  private applyEventToOrderProjection(
    ...
    switch (event.eventType) {
      ...
      case 'OrderItemAdded':
        ...
        updated.itemCount++;
        updated.totalAmount += itemAddedEvent.unitPrice * itemAddedEvent.quantity;
        updated.lastUpdated = event.timestamp;
        break;
      ...
      case 'OrderItemRemoved':
        ...
        updated.totalAmount = Math.max(0, updated.totalAmount - (itemRemovedEvent.unitPrice * itemRemovedEvent.quantity));
        updated.lastUpdated = event.timestamp;
        break;
  ...
  async buildCustomerOrderHistoryProjection(): Promise<Result<Map<string, CustomerOrderHistory>, Error>> {
    try {
      ...
      for (const orderProjection of orderProjections) {
        ...
        if (!history.lastOrderDate || orderProjection.createdAt > history.lastOrderDate) {
      ...
      return Result.fail(new Error(`Customer history projection failed: ${error.message}`));
    }
  }

  async buildRealTimeAnalyticsProjection(): Promise<Result<AnalyticsProjection, Error>> {
    try {
      ...
      const customerHistoryResult = await this.buildCustomerOrderHistoryProjection();
      
      if (customerHistoryResult.isSuccess()) {
        const customerHistories = Array.from(customerHistoryResult.value.values());
      ...
      return Result.fail(new Error(`Analytics projection failed: ${error.message}`));
    }
  }
  ...
  async notifyProjectionUpdates(streamId: string, event: DomainEvent): Promise<void> {
  ...
  async getOrderProjection(orderId: string): Promise<Result<OrderSummaryProjection | null, Error>> {
    try {
      const projection = this.projectionStore.get(`order-summary-${orderId}`);
      return Result.ok(projection || null);
    } catch (error) {
      return Result.fail(new Error(`Failed to get order projection: ${error.message}`));
    }
  }

  async getCustomerHistory(customerId: string): Promise<Result<CustomerOrderHistory | null, Error>> {
    try {
      ...
      return Result.fail(new Error(`Failed to get customer history: ${error.message}`));
    }
  }
  ...
  async seedTestData(): Promise<void> {
    ...
    for (const order of orders) {
      ...
      const events = [
        new OrderCreatedEvent(orderId, order.customerId, order.amount, 'USD', 
          Array(order.items).fill(null).map((_, i) => ({ id: `item-${i}`, price: order.amount / order.items }))
        ),
...
async function demonstrateStreamProjections() {
  ...
  const orderSummaryResult = await projectionEngine.buildOrderSummaryProjection();
  ...
  if (orderSummaryResult.isSuccess()) {
    ...
    for (let i = 1; i <= 5; i++) {
      const orderResult = await projectionEngine.getOrderProjection(i.toString());
      
      if (orderResult.isSuccess() && orderResult.value) {
        const order = orderResult.value;
        console.log(`    Order ${order.orderId}: $${order.totalAmount} (${order.status}) - ${order.itemCount} items`);
      }
    }
  }
  ...
  const customerHistoryResult = await projectionEngine.buildCustomerOrderHistoryProjection();
  ...
  if (customerHistoryResult.isSuccess()) {
    ...
    for (const [customerId, history] of customerHistories.entries()) {
      ...
      console.log(`    Average Order: $${history.averageOrderValue.toFixed(2)}`);
  ...
  if (analyticsResult.isSuccess()) {
    ...
    analytics.topCustomers.slice(0, 3).forEach(customer => {
      console.log(`    ${customer.customerId}: $${customer.totalSpent} (${customer.orderCount} orders)`);
    });
  }
  ...
  const newOrderEvents = [
    new OrderCreatedEvent(newOrderId, 'customer-4', 400, 'USD', [{ id: 'item-1', price: 400 }]),
    new OrderStatusChangedEvent(newOrderId, 'draft', 'confirmed')
  ];

  // Add events and notify subscriptions
  await projectionEngine.eventStore.appendEvents(`order-${newOrderId.value}`, newOrderEvents);
  
  for (const event of newOrderEvents) {
    await projectionEngine.notifyProjectionUpdates(`order-${newOrderId.value}`, event);
  }
  ...
  const customerHistResult = await projectionEngine.getCustomerHistory('customer-1');
...
- **Multi-View Support**: Different projections for different business requirements
- **Subscription System**: Real-time notifications for projection updates
- **Analytics Integration**: Built-in business intelligence and reporting capabilities
...
- **Incremental Updates**: Update projections incrementally rather than rebuilding
...
- **Projection Consistency**: Handle eventual consistency between write and read sides
