...
**Domain**: Multi-Service Integration
**Patterns**: Timeout Strategy, Adaptive Configuration, Performance Monitoring
**Dependencies**: @vytches-ddd/resilience

## Description

This example demonstrates advanced timeout strategies with adaptive configuration that dynamically adjusts timeout values based on historical performance, service health, and current load conditions. The system learns from past behavior to optimize timeout settings automatically.

## Business Context

A logistics management system integrates with multiple external services (shipping carriers, inventory systems, tracking APIs) that have varying response time characteristics and reliability patterns. Static timeouts lead to either premature cancellations or unnecessarily long waits. Adaptive timeouts improve both performance and reliability by learning service behavior patterns.
...
export class AdaptiveTimeoutManager {
  ...
  async executeWithAdaptiveTimeout<T>(
    ...
    if (!timeoutStrategy) {
      throw new Error(`No timeout strategy configured for service: ${serviceId}`);
    }
  ...
  private async calculateOptimalTimeout(serviceId: string): Promise<number> {
    ...
    const responseTimes = successfulCalls.map(m => m.duration).sort((a, b) => a - b);
    ...
    if (networkLatency > 100) { // High latency
    ...
    console.log(`Adaptive timeout for ${serviceId}: ${finalTimeout}ms (base: ${baseTimeout}ms, health: ${serviceHealth}, load: ${systemLoad.cpuUsage})`);
  ...
  private calculateServiceHealth(serviceId: string): number {
    ...
    const averageResponseTime = recentHistory
      .filter(m => m.success)
      .reduce((sum, m) => sum + m.duration, 0) / recentHistory.filter(m => m.success).length;
    ...
    const responseTimeHealth = baselineResponseTime > 0 
      ? Math.max(0, 1 - ((averageResponseTime - baselineResponseTime) / baselineResponseTime))
      : 1;
  ...
  private async recordPerformanceMetrics(serviceId: string, metrics: PerformanceMetrics): Promise<void> {
    ...
    if (!metrics.success || metrics.duration > (this.getBaselineResponseTime(serviceId) * 2)) {
  ...
  private initializeAdaptiveTimeouts(): void {
    ...
    services.forEach(service => {
      ...
      const timeoutStrategy = ResiliencePolicyBuilder
        .create()
        .withTimeout({
  ...
  getServiceMetrics(serviceId: string): ServiceMetrics {
    ...
    return {
      ...
      successRate: recentHistory.length > 0 ? successfulCalls.length / recentHistory.length : 0,
      averageResponseTime: successfulCalls.length > 0 
        ? successfulCalls.reduce((sum, m) => sum + m.duration, 0) / successfulCalls.length 
        : 0,
      currentTimeout: this.serviceTimeouts.get(serviceId)?.getCurrentTimeout() || 0,
  ...
  async manuallyAdjustTimeout(serviceId: string, newTimeout: number): Promise<void> {
    ...
    if (newTimeout < config.minTimeout || newTimeout > config.maxTimeout) {
      throw new Error(`Timeout ${newTimeout}ms outside bounds [${config.minTimeout}, ${config.maxTimeout}]`);
    }
...
async function processShipmentTracking(trackingNumber: string) {
  ...
  try {
    const trackingInfo = await timeoutManager.executeWithAdaptiveTimeout(
      ...
      async () => {
        const response = await fetch(`https://tracking-api.com/track/${trackingNumber}`);
...
setInterval(() => {
  ...
  metrics.forEach(metric => {
    console.log(`${metric.serviceId}: ${metric.currentTimeout}ms timeout, ${(metric.successRate * 100).toFixed(1)}% success, ${metric.averageResponseTime.toFixed(0)}ms avg response`);
  });
}, 60000); // Every minute
...
- **Adaptive Timeout Calculation**: Dynamically adjusts based on historical performance
