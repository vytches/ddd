...
**Domain**: Health-Aware Service Management
...
This example demonstrates intermediate NestJS integration with adaptive resilience patterns that automatically adjust based on service health monitoring. The system continuously monitors service performance and adapts resilience configurations for optimal protection and performance.

## Business Context

A distributed service platform manages multiple microservices with varying performance characteristics. The system needs intelligent resilience that adapts to changing service health, automatically tightening protection during degradation and relaxing constraints during healthy periods for optimal performance.
...
export class AdaptiveResilienceService implements OnModuleInit {
  ...
  async executeServiceRequest(
    ...
    if (!strategy) {
      throw new Error(`No resilience strategy configured for service: ${serviceId}`);
    }
    ...
    try {
      ...
      this.logger.debug(`Service request successful: ${serviceId}/${request.operationType}`);
      return result;
      ...
      this.logger.error(`Service request failed: ${serviceId}/${request.operationType}`, error.stack);
      throw error;
    }
  }
  ...
  private async performHealthBasedAdaptation(): Promise<void> {
    ...
    for (const [serviceId, strategy] of this.serviceStrategies) {
      try {
        ...
        if (await this.shouldAdaptStrategy(serviceId, currentHealth, previousHealth)) {
          const adaptations = await this.generateAdaptations(serviceId, currentHealth, strategy);
        ...
        this.logger.error(`Health adaptation failed for service ${serviceId}:`, error);
      }
    }
  }
  ...
  private async initializeAdaptiveStrategies(): Promise<void> {
    ...
    const userServiceStrategy = ResiliencePolicyBuilder
      .createHealthAware()
    ...
    const paymentServiceStrategy = ResiliencePolicyBuilder
      .createHealthAware()
      .withSmartCircuitBreaker({
      ...
      .withHealthLearning({
        learningEnabled: true,
        adaptationThreshold: 0.10, // 10% health change (more sensitive)
        stabilizationPeriod: 90000  // 1.5 minute stabilization
      })
      .build();

    // Inventory service - aggressive adaptation
    const inventoryServiceStrategy = ResiliencePolicyBuilder
      .createHealthAware()
      .withSmartCircuitBreaker({
      ...
      .withHealthLearning({
        learningEnabled: true,
        adaptationThreshold: 0.20, // 20% health change (less sensitive)
    ...
    this.logger.log('Adaptive resilience strategies initialized for 3 services');
  }
  ...
  private async assessServiceHealth(serviceId: string): Promise<HealthSnapshot> {
    const strategy = this.serviceStrategies.get(serviceId);
    if (!strategy) {
    ...
    try {
      ...
      const totalRequests = metrics.successfulExecutions + metrics.failedExecutions;
      const successRate = totalRequests > 0 ? metrics.successfulExecutions / totalRequests : 1;
      const errorRate = totalRequests > 0 ? metrics.failedExecutions / totalRequests : 0;
      
      // Calculate composite health score
      const healthScore = this.calculateHealthScore(successRate, responseTime, errorRate);
  ...
  private calculateHealthScore(
    ...
    return Math.max(0, Math.min(1, successFactor + responseFactor + errorFactor));
  }
  ...
  private async shouldAdaptStrategy(
    ...
    const healthChange = Math.abs(currentHealth.healthScore - previousHealth.healthScore);
    ...
    return healthChange >= config.adaptationThreshold && 
  ...
  private async generateAdaptations(
    ...
    if (health.healthScore < 0.5) {
      ...
      adaptations.push({
        ...
        changes: {
          failureThreshold: Math.max(2, Math.floor(currentConfig.circuitBreaker.failureThreshold * 0.7)),
      ...
      adaptations.push({
        type: 'circuit-breaker',
        changes: {
          failureThreshold: Math.min(10, Math.floor(currentConfig.circuitBreaker.failureThreshold * 1.2)),
          resetTimeout: Math.max(15000, currentConfig.circuitBreaker.resetTimeout * 0.8)
        },
    ...
    if (health.responseTime > currentConfig.timeout.baseTimeout * 1.2) {
      ...
      adaptations.push({
        type: 'timeout',
        changes: {
          defaultTimeout: Math.min(currentConfig.timeout.maxTimeout, health.responseTime * 1.5)
        },
      ...
      adaptations.push({
        type: 'timeout',
        changes: {
          defaultTimeout: Math.max(currentConfig.timeout.minTimeout, health.responseTime * 2)
        },
  ...
  private async applyAdaptations(
    ...
    for (const adaptation of adaptations) {
      try {
        ...
        this.logger.log(`Applied ${adaptation.type} adaptation for ${serviceId}: ${adaptation.reasoning}`);
      } catch (error) {
        this.logger.error(`Failed to apply ${adaptation.type} adaptation for ${serviceId}:`, error);
      }
    }
  }
  ...
  private async callExternalService(serviceId: string, request: ServiceRequest): Promise<any> {
  ...
  private async performHealthCheck(serviceId: string): Promise<void> {
    ...
    if (Math.random() < 0.05) { // 5% health check failure rate
  ...
  private async recordExecutionSuccess(serviceId: string, context: any, result: any): Promise<void> {
    // Record successful execution metrics
  }

  private async recordExecutionFailure(serviceId: string, context: any, error: Error): Promise<void> {
  ...
  private getMostAdaptiveService(): string {
    ...
    this.adaptationHistory.forEach(a => {
      serviceAdaptations.set(a.serviceId, (serviceAdaptations.get(a.serviceId) || 0) + 1);
    });
  ...
  private getAverageHealthScore(): number {
    const healthScores = Array.from(this.healthSnapshots.values()).map(h => h.healthScore);
  ...
  private calculateAdaptationFrequency(): number {
    ...
    const timeSpan = this.adaptationHistory[this.adaptationHistory.length - 1].timestamp.getTime() - 
                    this.adaptationHistory[0].timestamp.getTime();
    
    return this.adaptationHistory.length / (timeSpan / (60 * 1000)); // adaptations per minute
  }

  private determineTriggerReason(health: HealthSnapshot, adaptations: any[]): string {
...
export class AdaptiveResilienceController {
  constructor(private readonly adaptiveResilienceService: AdaptiveResilienceService) {}
  ...
  async executeServiceRequest(
    ...
    return await this.adaptiveResilienceService.executeServiceRequest(serviceId, request);
  }
...
- **Health-Aware Adaptation**: Automatically adjusts resilience based on service health
- **Smart Circuit Breaker**: Adaptive failure thresholds based on health metrics
- **Performance-Based Timeouts**: Dynamic timeout adjustment based on response times
...
- **Stabilization Too Short**: Not allowing enough time for adaptations to take effect
