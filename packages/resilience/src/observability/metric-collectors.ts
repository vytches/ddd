...
export class CircuitBreakerMetricCollector extends BaseMetricCollector {
  ...
  constructor(instanceName: string, labels: MetricLabels = {}) {
    super(`circuit_breaker_${instanceName}`, { pattern: 'circuit_breaker', instance: instanceName, ...labels });
  ...
  recordExecution(success: boolean, executionTime: number): void {
    ...
    this.metrics = {
      ...this.metrics,
      avgExecutionTime: this.executionTimes.reduce((sum, time) => sum + time, 0) / this.executionTimes.length
    };
  }

  recordStateChange(newState: 'CLOSED' | 'OPEN' | 'HALF_OPEN', oldState: 'CLOSED' | 'OPEN' | 'HALF_OPEN'): void {
  ...
  collect(): ReadonlyArray<Metric | HistogramMetric | TimerMetric> {
    ...
    return [
      this.createMetric('execution_total', this.metrics.executionCount, 'counter', 'Total executions', stateLabels),
      this.createMetric('success_total', this.metrics.successCount, 'counter', 'Successful executions', stateLabels),
      this.createMetric('failure_total', this.metrics.failureCount, 'counter', 'Failed executions', stateLabels),
      this.createMetric('rejected_total', this.metrics.rejectedCount, 'counter', 'Rejected executions', stateLabels),
      this.createMetric('state_transitions_total', this.metrics.openToHalfOpenCount, 'counter', 'Open to Half-Open transitions', { transition: 'open_to_half_open' }),
      this.createMetric('state_transitions_total', this.metrics.halfOpenToClosedCount, 'counter', 'Half-Open to Closed transitions', { transition: 'half_open_to_closed' }),
      this.createMetric('state_transitions_total', this.metrics.halfOpenToOpenCount, 'counter', 'Half-Open to Open transitions', { transition: 'half_open_to_open' }),
      this.createTimerMetric('execution_duration', this.metrics.lastExecutionTime, 'ms', 'Last execution duration', stateLabels),
      this.createTimerMetric('execution_duration_avg', this.metrics.avgExecutionTime, 'ms', 'Average execution duration', stateLabels),
...
/**
 * Retry pattern metrics collector
 */
export class RetryMetricCollector extends BaseMetricCollector {
  ...
  recordExecution(success: boolean, executionTime: number, retryCount: number, maxRetriesReached: boolean): void {
    this.metrics = {
      ...this.metrics,
      executionCount: this.metrics.executionCount + 1,
      successCount: this.metrics.successCount + (success ? 1 : 0),
    ...
    this.metrics = {
      ...this.metrics,
      avgExecutionTime: this.executionTimes.reduce((sum, time) => sum + time, 0) / this.executionTimes.length,
      avgRetryCount: this.retryAttempts.reduce((sum, count) => sum + count, 0) / this.retryAttempts.length
    };
  }

  collect(): ReadonlyArray<Metric | HistogramMetric | TimerMetric> {
    return [
      this.createMetric('execution_total', this.metrics.executionCount, 'counter', 'Total executions'),
      this.createMetric('success_total', this.metrics.successCount, 'counter', 'Successful executions'),
      this.createMetric('failure_total', this.metrics.failureCount, 'counter', 'Failed executions'),
      this.createMetric('retry_attempts_total', this.metrics.totalRetryCount, 'counter', 'Total retry attempts'),
      this.createMetric('max_retries_reached_total', this.metrics.maxRetriesReachedCount, 'counter', 'Max retries reached count'),
      this.createMetric('retry_attempts_avg', this.metrics.avgRetryCount, 'gauge', 'Average retry attempts per execution'),
      this.createTimerMetric('execution_duration', this.metrics.lastExecutionTime, 'ms', 'Last execution duration'),
      this.createTimerMetric('execution_duration_avg', this.metrics.avgExecutionTime, 'ms', 'Average execution duration'),
      this.createHistogramMetric(
...
/**
 * Bulkhead pattern metrics collector
 */
export class BulkheadMetricCollector extends BaseMetricCollector {
  ...
  constructor(instanceName: string, maxConcurrency: number, queueCapacity: number, labels: MetricLabels = {}) {
  ...
  collect(): ReadonlyArray<Metric | HistogramMetric | TimerMetric> {
    return [
      ...
      this.createMetric('success_total', this.metrics.successCount, 'counter', 'Successful executions'),
      this.createMetric('failure_total', this.metrics.failureCount, 'counter', 'Failed executions'),
      this.createMetric('rejected_total', this.metrics.rejectedExecutions, 'counter', 'Rejected executions'),
      this.createMetric('active_executions', this.metrics.activeExecutions, 'gauge', 'Current active executions'),
      this.createMetric('queued_executions', this.metrics.queuedExecutions, 'gauge', 'Current queued executions'),
      this.createMetric('max_concurrency', this.metrics.maxConcurrency, 'gauge', 'Maximum allowed concurrency'),
      this.createMetric('queue_capacity', this.metrics.queueCapacity, 'gauge', 'Queue capacity'),
      this.createMetric('utilization', this.metrics.activeExecutions / this.metrics.maxConcurrency, 'gauge', 'Concurrency utilization ratio'),
      this.createTimerMetric('execution_duration', this.metrics.lastExecutionTime, 'ms', 'Last execution duration'),
      this.createTimerMetric('execution_duration_avg', this.metrics.avgExecutionTime, 'ms', 'Average execution duration')
    ];
  }
...
/**
 * Timeout pattern metrics collector
 */
export class TimeoutMetricCollector extends BaseMetricCollector {
  ...
  recordExecution(success: boolean, executionTime: number, timedOut: boolean, timeoutDuration?: number): void {
    this.metrics = {
      ...this.metrics,
      executionCount: this.metrics.executionCount + 1,
      successCount: this.metrics.successCount + (success ? 1 : 0),
    ...
    this.metrics = {
      ...this.metrics,
      avgExecutionTime: this.executionTimes.reduce((sum, time) => sum + time, 0) / this.executionTimes.length,
      avgTimeoutDuration: this.timeoutDurations.length > 0
        ? this.timeoutDurations.reduce((sum, duration) => sum + duration, 0) / this.timeoutDurations.length
        : 0
    };
  }
  ...
  collect(): ReadonlyArray<Metric | HistogramMetric | TimerMetric> {
    return [
      ...
      this.createMetric('success_total', this.metrics.successCount, 'counter', 'Successful executions'),
      this.createMetric('failure_total', this.metrics.failureCount, 'counter', 'Failed executions'),
      this.createMetric('timeout_total', this.metrics.timeoutCount, 'counter', 'Timeout occurrences'),
      this.createTimerMetric('execution_duration', this.metrics.lastExecutionTime, 'ms', 'Last execution duration'),
      this.createTimerMetric('execution_duration_avg', this.metrics.avgExecutionTime, 'ms', 'Average execution duration'),
      this.createTimerMetric('timeout_duration_avg', this.metrics.avgTimeoutDuration, 'ms', 'Average timeout duration')
    ];
  }
