...
function buildFooter(packages) {
  return [
    ...
    `  \`@vytches/ddd-lint\`.`,
    `- Aggregate state mutation must go through \`apply()\` (string form:`,
    `  \`apply('EventName', payload)\` or class form: \`apply(new MyEvent(payload))\`)`,
    `  — never assign fields directly. The class form preserves \`instanceof\``,
    `  checks and is preferred when using typed DomainEvent subclasses.`,
