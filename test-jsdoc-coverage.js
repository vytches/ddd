...
function checkJSDocCoverage(dir) {
  ...
  function processFile(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        line.match(
          /^export (class|interface|type|enum|function|const declare|declare class|declare function)/
        )
      ) {
        results.total++;
        ...
        const nameMatch = line.match(
          /(?:class|interface|type|enum|function|const|declare class|declare function)\s+([a-zA-Z0-9_]+)/
        );
