...
class JSDocCoverageVerifier {
  ...
  shouldHaveJSDoc(node) {
    ...
    const isClassMember = node.parent && (
  ...
  getNodeType(node) {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.PropertyDeclaration: return 'property';
      ...
      case ts.SyntaxKind.VariableStatement: return 'const';
   ...
   */
  generateReport() {
    const totalCoverage = this.results.total > 0 
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    ...
    console.log('  node verify-jsdoc-coverage.js packages/resilience/dist/observability/metric-registry.d.ts');
    console.log('  node verify-jsdoc-coverage.js packages/resilience/dist/observability/metric-registry.d.ts --yaml docs/examples/domain/resilience/observability/metric-registry.yaml');
    process.exit(1);
  }
