...
const rules = {
  'no-any-regression': {
    meta: {
      ...
      messages: {
        anyRegression: 'New `any` type usage detected. This violates the quality gate for preventing any type regression.',
        unjustifiedAny: 'Unjustified `any` type usage. Consider using a more specific type or add to infrastructure patterns if this is a valid use case.',
        documentJustification: 'If this `any` usage is justified (decorators, event constructors, etc.), please document why in a comment.'
      }
    },
  ...
  'prefer-unknown-over-any': {
    meta: {
      ...
      messages: {
        preferUnknown: 'Prefer `unknown` over `any` for better type safety. Use `any` only when absolutely necessary.'
      }
    },
