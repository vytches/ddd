...
function generateReport() {
  ...
  console.log(
    `${colors.bold}${colors.cyan}        VytchesDDD JSDoc Verification Report${colors.reset}`
  );
  console.log(
    `${colors.bold}${colors.cyan}═══════════════════════════════════════════════════════════════${colors.reset}\n`
  );
  ...
  console.log(
    `  JSDoc auto-injection: ${buildInfo.hasJSDocInject ? colors.green + '✓' : colors.red + '✗'} ${colors.reset}`
  );
  console.log(
    `  Clean build available: ${buildInfo.hasCleanBuild ? colors.green + '✓' : colors.red + '✗'} ${colors.reset}`
  );
  console.log(
    `  Build command: ${colors.cyan}${buildInfo.buildCommand?.substring(0, 80)}...${colors.reset}\n`
  );
  ...
  for (const packageName of packages) {
    ...
    const metadata = packageInfo[packageName] || {
      priority: 'UNKNOWN',
      complexity: 'UNKNOWN',
      status: 'UNKNOWN',
    };
  ...
  // Display results
  console.log(
    `${colors.bold}Package${' '.repeat(18)}Priority  Complex  Status       JSDoc   YAML  Last Build${colors.reset}`
  );
  ...
  for (const result of results) {
    ...
    const coverageColor =
      result.dist.coverage >= 80
        ? colors.green
        : result.dist.coverage >= 50
          ? colors.yellow
          : colors.red;

    const yamlColor = result.yaml.files > 0 ? colors.green : colors.red;

    const lastBuildStr = result.lastBuild
      ? new Date(result.lastBuild).toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          hour: '2-digit',
          minute: '2-digit',
  ...
  console.log(
    `  Overall JSDoc coverage: ${overallCoverage}% (${totalWithJSDoc}/${totalFiles} files)`
  );
  console.log(`  Packages with YAML metadata: ${packagesWithYAML}/${packages.length}`);
  console.log(`  Total YAML files: ${totalYAMLFiles}`);
  console.log(
    `  Status: ${colors.green}${packagesDone} Done${colors.reset}, ${colors.yellow}${packagesInProgress} In Progress${colors.reset}, ${colors.red}${packagesTodo} TODO${colors.reset}\n`
  );
  ...
  if (inProgress.length > 0) {
    ...
    for (const pkg of inProgress) {
      console.log(
        `  • ${colors.yellow}${pkg.name}${colors.reset} - Coverage: ${pkg.dist.coverage}%, YAML files: ${pkg.yaml.files}`
      );
    }
  }
  ...
  const lowCoverage = results.filter(
    r => r.dist.files > 0 && r.dist.coverage < 50 && r.status !== 'TODO'
  );
...
function generateDetailedReport(results, outputPath) {
  ...
  for (const result of results) {
    const statusEmoji =
      result.status === 'DONE' ? '✅' : result.status === 'IN_PROGRESS' ? '🔄' : '❌';
